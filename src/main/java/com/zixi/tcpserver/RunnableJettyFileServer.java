...
public class RunnableJettyFileServer implements Runnable{
	
	// The file server handler class. 
	static class CustmHeaderResourceHandler extends HandlerWrapper implements ResourceFactory,WelcomeFactory
	{
	    ...
	    /* ------------------------------------------------------------ */
	    public CustmHeaderResourceHandler(ResourceService resourceService)
	    {
	        _resourceService=resourceService;
	    }

	    /* ------------------------------------------------------------ */
	    public CustmHeaderResourceHandler(Object writeLock)
	    {
	    ...
	    {						
	    	...
	    	//response.addHeader("Access-Control-Allow-Headers", "origin,range,accept-encoding,referer");
	    	response.addHeader("Access-Control-Allow-Headers", "origin,range,accept-encoding,referer");
	    	response.addHeader("Access-Control-Expose-Headers", "Server,range,Content-Length,Content-Range,Geo-Info,Quic-Version");
	    	response.addHeader("Accept-Ranges", "bytes");
	    	response.addHeader("Access-Control-Max-Age", "600");
	    	response.addHeader("Connection", "keep-alive");
	    	response.addHeader("ETag", "242194954f133343cacee93353e805ae:1458324924");
	    	
	    	String path = request.getPathInfo().split("/")[request.getPathInfo().split("/").length - 1];
	    	String ext  = path.split("\\.")[path.split("\\.").length -1];
			switch(ext)
	    	{
			 	case("mpd"): response.addHeader("Content-Type", "application/dash+xml");
			 		break;
			 	case("m4v"): response.addHeader("Content-Type", "video/mp4");
			 		break;
			 	case("m4a"): response.addHeader("Content-Type", "audio/mp4");
		 			break;
			 	case("m3u8"): response.addHeader("Content-Type", "audio/x-mpegurl");
			 		break;
			 	case("ts"): response.addHeader("Content-Type", "video/mp2t");
		 			break;
		 		default:	 response.addHeader("Content-Type", "application/dash+xml");
			 }
	...
	public RunnableJettyFileServer(Server fileServer, String fileBase, Object writeLock)
	{
	...
	public void run()
	{
		...
		CustmHeaderResourceHandler fileResource_handler = new CustmHeaderResourceHandler(writeLock);

        // Configure the ResourceHandler. Setting the resource base indicates where the files should be served out of.
        // In this example it is the current directory but it can be configured to anything that the jvm has access to.
        fileResource_handler.setDirectoriesListed(true);
        fileResource_handler.setWelcomeFiles(new String[]{ "index.html" });
        fileResource_handler.setResourceBase(fileBase + "/");
       
        // Add the ResourceHandler to the server.
        HandlerList handlers = new HandlerList();
        handlers.setHandlers(new Handler[] { fileResource_handler,  new DefaultHandler() });
