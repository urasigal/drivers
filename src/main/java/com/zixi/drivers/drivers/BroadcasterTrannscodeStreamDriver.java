...
public class BroadcasterTrannscodeStreamDriver extends BroadcasterLoggableApiWorker implements TestDriver {
	...
	public String testIMPL(String userName, String userPass, String login_ip,
		...
		"rec_duration:" + rec_duration, "src:" + src, "vp:" + "   ", "ap:" + ap, "bit:" + bit);
	...
	// Input stream transcoding.
	public DriverReslut testIMPL(String userName, String userPass, String login_ip, String uiport, String type,String id,
	...
	String bit, String profile_name, String mode,  String vp, String smoothing, String max_va_diff_ms) throws Exception {
		...
		String url = null;
		if(profile_name.equals("-1"))
		{
			url = "http://" + login_ip + ":" + uiport + "/zixi/add_stream.json?" + "type=" + type + "&id=" + id
			+ "&matrix=" + matrix + "&max_outputs=" + max_outputs + "&mcast_out=" + mcast_out + "&time_shift=" + time_shift
			+ "&old=" + old + "&fast_connect=" + fast_connect + "&kompression=" + kompression + "&enc_type=" + enc_type
			+ "&enc_key=" + enc_key + "&rec_history=" + rec_history + "&rec_duration=" + rec_duration + "&src=" + src +
			"&ap=" + audioProfile + "&ll=" + ll+ "&all_pids=" + all_pids + "&bit=" + bit  + "&use_hw=" + use_hw + "&vp=" +vp +
			"&smoothing=" + smoothing + "&max_va_diff_ms=" + max_va_diff_ms;
				
		}else
			url = "http://" + login_ip + ":" + uiport + "/zixi/add_stream.json?" + "type=" + type + "&id=" + id
			+ "&matrix=" + matrix + "&rmax_outputs=" + max_outputs + "&mcast_out="+ mcast_out + "&time_shift=" + time_shift
			+ "&old=" + old + "&fast_connect=" + fast_connect + "&kompression=" + kompression + "&enc_type=" + enc_type
			+ "&enc_key=" + enc_key + "&rec_history=" + rec_history + "&rec_duration=" + rec_duration + "&src=" + src + "&vp=" + pid + "&ap="
			+ audioProfile + "&ll=" + ll+ "&all_pids=" + all_pids + "&bit=" + bit  + "&use_hw=" + use_hw + "&vp=" +vp + "&smoothing=" + smoothing +
			"&max_va_diff_ms=" + max_va_diff_ms;
		
		return new DriverReslut(apiworker.sendGet(url, "", ADD_TRANSCODER_PROFILE, responseCookieContainer, login_ip, this, uiport));
	}
