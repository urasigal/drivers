...
public class ReceiverUdpOutCreationDriver extends BroadcasterLoggableApiWorker implements TestDriver {

	//private ApiWorkir streamCreator = new ApiWorkir();

	//private TestParameters testParameters;

	public DriverReslut testIMPL(String userName, String userPass, String login_ip, String uiport, String name, String target, String type, String nic,
	String ttl, String smoothing, String rtp, String fec, String rows, String cols, String remux_bitrate,String input_stream) throws Exception {
...
//		
		...
		// Get stream id.
		String streamId = apiworker.sendGet("http://" + login_ip + ":" + uiport + "/in_streams.json?complete=1", input_stream, RECEIVERIDMODE,
		responseCookieContainer, login_ip, this, uiport);

		return new DriverReslut( apiworker.sendGet("http://" + login_ip + ":" + uiport + "/add_output.json?" + rname + "=" + name + "&" + rtarget
		+ "=" + target + "&" + rtype + "=" + type + "&" + rnic + "=" + nic + "&" + rttl + "=" + ttl + "&" + rsmoothing + "="
		+ smoothing + "&" + rrtp + "=" + rtp + "&" + rfec + "=" + fec+ "&" + rrows + "=" + rows + "&" + rcols + "=" + cols + "&"
		+ rremux_bitrate + "=" + remux_bitrate +"&"+ rin_id +"="+ streamId , "", RECEIVER_UDP_OUT_MODE, responseCookieContainer , login_ip, this, uiport));
	}
	...
	{
		responseCookieContainer = broadcasterInitialSecuredLogin.sendGet( "http://" + receiver_ip + ":" + recever_uiport + "/login.htm", receiver_user_name,
		receiver_password, receiver_ip, recever_uiport);
		
		return apiworker.sendGet("http://" + receiver_ip +  ":" + recever_uiport + "/out_add_in.json?out_id=" + mainId + "&source=" + backupId + "&fallback=1",
